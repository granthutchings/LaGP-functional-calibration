...
tCalib = as.matrix(expand.grid(ux,ux))
calib = mv.calib.nobias(tCalib,
         SCinputs,
         estLS,
         Ydata,
         simBasis$Vt,
...
```{r , echo=F}
emPred = mv.em.pred(calib$theta.hat,Xobs[1:n,,drop=F],estLS,XTdata,simBasis$Vt)
Ypred = yPred.nobias(emPred$GPs,calib$ssq.hat,obsBasis$B,BtBinv,1000,Ydata$obs, ysamp = T)
...
emPredNew = mv.em.pred(calib$theta.hat,XobsNew,estLS,XTdataNew,simBasis$Vt)
YpredNew = yPred.nobias(emPredNew$GPs,calib$ssq.hat,obsBasisNew$B,BtBinvNew,1000,YdataNew$obs,ysamp = T)
