...
# calib = mv.calib.nobias(tCalib = tCalib,
#                         SCinputs,
#                         estLS,
#                         Ydata,
#                         simBasis$Vt,
...
#                         BtBinv,
#                         thetaHat=T)
calib = mv.calib.nobias_yllh(tCalib = tCalib,
                      SCinputs,
                      estLS,
                      Ydata,
                      simBasis$Vt,
...
ggplot(calib$ll_df, aes(x = theta1, y = theta2, col = ll)) +
     geom_point(size = 3) +
     scale_color_viridis_c() + geom_point(aes(x=calib$theta.hat[1],y=calib$theta.hat[2]),col='red',size=1) +
...
```{r}
emGPs = mv.em.pred(calib$theta.hat.orig,Xobs[1:n,,drop=F],estLS,XTdata,simBasis$Vt)
Ypred = yPred.nobias(emGPs$GPs,calib$ssq.hat,obsBasis$B,BtBinv,nsamples=1000,Ydata$obs,ysamp=F)
```
```{r , echo=F}
for(i in 1:n){
  matplot(YindObs[,1],Ydata$obs$orig[,obs_ids[[i]],drop=F],pch=1,col=i+1,add=ifelse(i==1,F,T),ylim=c(0,10),
...
#emGPs = mv.em.pred(calib$theta.hat.orig,Xobs[1:n,,drop=F],estLS,XTdata,simBasis$Vt)
Ypred = yPred.nobias(emGPs$GPs,calib$ssq.hat,obsBasis$B,BtBinv,nsamples=1000,Ydata$obs,ysamp=T)
for(i in 1:n){
  matplot(YindObs[,1],Ydata$obs$orig[,obs_ids[[i]],drop=F],pch=1,col=i+1,add=ifelse(i==1,F,T),ylim=c(0,10),
          xlab='distance',ylab='time',main='')
  points(YindObs[,1],Ytrue[,i],col='black')
...
emGPsNew = mv.em.pred(calib$theta.hat.orig,XobsNew,estLS,XTdataNew,simBasis$Vt)
YpredNew = yPred.nobias(emGPsNew$GPs,calib$ssq.hat,
                    obsBasisNew$B,BtBinvNew,nsamples=1000,YdataNew$obs,ysamp=F)
...
```{r manual predictions, echo=F}
wMean = emGPsNew$GPs$wMean; wVar = emGPsNew$GPs$wVar
ssq.hat = calib$ssq.hat
B = obsBasisNew$B
nY = nrow(YindObs)
nsamples=1000

Ysamp = (B%*%t(mvtnorm::rmvnorm(nsamples,mean=wMean[,1,drop=F],sigma =diag(wVar[,1]))) + 
    t(mvtnorm::rmvnorm(nsamples,sigma=ssq.hat*eye(nY)))) #* YdataNew$obs$sd + YdataNew$obs$mean
Yvar = apply(Ysamp,1,var)
#round(mean(sqrt(Yvar)),4)

Ysamp2 = B%*%BtBinvNew%*%t(B)%*%Ysamp
Yvar2 = apply(Ysamp2,1,var)
#round(mean(sqrt(Yvar2)),4)

Ysamp3 = B%*%t(mvtnorm::rmvnorm(nsamples,mean=wMean[,1,drop=F],sigma =diag(wVar[,1])+ssq.hat*BtBinvNew))
Yvar3 = apply(Ysamp3,1,var)
#round(mean(sqrt(Yvar3)),4)

Ysamp4 = (B%*%t(mvtnorm::rmvnorm(nsamples,mean=wMean[,1,drop=F],sigma =diag(wVar[,1]))) + 
    B%*%BtBinvNew%*%t(B)%*%t(mvtnorm::rmvnorm(nsamples,sigma=ssq.hat*eye(nY)))) #* YdataNew$obs$sd + YdataNew$obs$mean
Yvar4 = apply(Ysamp4,1,var)
#round(mean(sqrt(Yvar4)),4)
```
