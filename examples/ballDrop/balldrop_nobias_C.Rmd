...
```{r mcmc, echo=F}
# compare this to mcmc dist
mcmc.out = mcmc(mvcData,tInit=init$theta1[which.max(init$ll)],bias=F,nsamples=10000,nburn=1000,prop.var = .08)
plot(mcmc.out$t.samp,type='l')
plot(mcmc.out$ssq.samp,type='l')
hist(mcmc.out$t.samp);abline(v=mean(mcmc.out$t.samp),col='red')
...
yPred = ypred_mcmc(mvcData$XTdata$obs$X$orig[1:n,,drop=F],mvcData,tsamp,support=support,bias=F)
for(i in 1:n){
...
py_save_object(mvcDataNew,'bd_nobias_oos_C.pkl')

# MCMC
yPredMCMC = ypred_mcmc(XobsNew,mvcDataNew,tsamp,support=support,bias=F)

# Point Estimate
yPredPoint = ypred_mle(XobsNew,mvcDataNew,calib,1000,support=support,bias=F)
