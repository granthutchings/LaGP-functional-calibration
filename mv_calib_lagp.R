...
{## Parameters:
  ...
  # optional
  # support: string, one of 'obs' or 'sim' indicating if predictions should be returned at the indices y.ind.obs or y.ind.sim
...
get_obs_basis = function(sim.basis,Y.obs,y.ind.sim,y.ind.obs,sigma.y=NULL){
  ...
  # if(!is.null(sigma.y)){
  #   lamY = (1/sigma.y)*eye(dim(obs.basis$B)[1])
  # } else{
  #   lamY = eye(dim(obs.basis$B)[1])
  # }
  # Bprod = t(obs.basis$B)%*%lamY%*%obs.basis$B
  # obs.basis$V.t = solve(Bprod + Bridge) %*% t(obs.basis$B)%*%lamY%*%Y.obs
...
transform_y = function(Y.sim,y.ind.sim=NULL,Y.obs=NULL,y.ind.obs=NULL,center=T,scale=T, scaletype='rowwise'){
  sim.list = list()
  obs.list = list()
  ...
  if(center){
    ...
    sim.list$mean = rep(0,nrow(Y.sim))
  }
  if(scale){
    if(scaletype=='rowwise'){
      sim.list$sd = apply(Y.sim,1,sd)
      # dont scale places with var 0
      sim.list$sd[sim.list$sd==0]=1
    } else if(scaletype=='scalar') {
      sd = sd(Y.sim)
      sim.list$sd = rep(sd,nrow(Y.sim))
    }
  } else{
    sim.list$sd = rep(1,nrow(Y.sim))
  }
  sim.list$trans = t(scale(t(Y.sim),sim.list$mean,sim.list$sd))
  
  # scale Y.obs
  if(!is.null(Y.obs)){
    obs.list$orig = Y.obs
    obs.list$n.y = dim(Y.obs)[1]
    if(!isTRUE(all.equal(y.ind.obs,y.ind.sim))){
      # interpolation is needed
      if(dim==1){
        # 1d interpolation of sim mean onto yobs support
        obs.list$mean = interp1(as.numeric(y.ind.sim),sim.list$mean,as.numeric(y.ind.obs))
        obs.list$sd = interp1(as.numeric(y.ind.sim),sim.list$sd,as.numeric(y.ind.obs))
  ...
  sim.list$y.ind.sim = y.ind.sim
  sim.list$n.y = nrow(Y.sim)
  obs.list$y.ind.obs = y.ind.obs
...
get_SC_inputs = function(est.ls,XT.data,n.pc){
  ...
  if(length(est.ls$X)>0){
    X.sim = lapply(1:n.pc, function(i) sc_inputs(XT.data$sim$X$trans,est.ls$X[[i]]))
  } else{
    X.sim = NULL
  }
  if(length(est.ls$T)>0){
    T.sim =  lapply(1:n.pc, function(i) sc_inputs(XT.data$sim$T$trans,est.ls$T[[i]]))
  } else{
    T.sim = NULL
  }
...
energy_score_snomadr = function(theta,mvc.data,sample=T,n.samples=100,start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50){
  ...
  if(bias){
    ...
    if(laGP.delta){ # use laGP for a faster bias model
...
interval_score_snomadr = function(theta,mvc.data,sample=T,n.samples=100,start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50,
                                  lite=T){
  n.pc = nrow(mvc.data$obs.basis$V.t)
  n = nrow(mvc.data$SC.inputs$X.obs[[1]])
  n.y = nrow(mvc.data$Y.data$obs$orig)
...
mvc_data = function(X.sim=NULL,T.sim=NULL,X.obs=NULL,T.obs=NULL,                                # X and T data
  ...
  if(precomp){
    ...
    BD = cbind(obs.basis$B,obs.basis$D)
    tBD = t(BD)
    precomp$rankBD = rankMatrix(BD)
    BDtBD = tBD%*%BD
    if(precomp$rankBD != ncol(BD)){
      precomp$BDtBDinv = solve(BDtBD + 1e-8*eye(nrow(BDtBD)))
    } else{
      precomp$BDtBDinv = solve(BDtBD) # need to get rid of solves to make code faster
    }
    ...
    precomp$Z = lapply(1:sim.basis$n.pc,function(jj) sim.basis$V.t[jj,])
...
get_calib_data = function(mvc.data){
  
  # store the objects needed for calibration in an object smaller than mvc.data, which can be inefficient to pass around through functions
  mvc.data = mvc.data
  ...
  mvc.data$n = mvc.data$Y.data$n
  mvc.data$n.y = mvc.data$Y.data$obs$n.y
  mvc.data$I.n.y = eye(mvc.data$n.y)
  mvc.data$BD = cbind(mvc.data$obs.basis$B,mvc.data$obs.basis$D)
  mvc.data$tBD = t(mvc.data$BD)
  mvc.data$rankBD = rankMatrix(mvc.data$BD)
  mvc.data$BDtBD = mvc.data$tBD%*%mvc.data$BD
  if(mvc.data$rankBD != ncol(mvc.data$BD)){
    mvc.data$BDtBDinv = solve(mvc.data$BDtBD + 1e-8*eye(nrow(mvc.data$BDtBD)))
  } else{
    mvc.data$BDtBDinv = solve(mvc.data$BDtBD) # need to get rid of solves to make code faster
  }
  mvc.data$ldetBDtBD = determinant(mvc.data$BDtBD)$modulus
  mvc.data$BDtBDinvtBD = mvc.data$BDtBDinv%*%mvc.data$tBD
  mvc.data$LLHmat = mvc.data$I.n.y-mvc.data$BD%*%mvc.data$BDtBDinv%*%mvc.data$tBD
  mvc.data$Z = lapply(1:mvc.data$sim.basis$n.pc,function(jj) mvc.data$sim.basis$V.t[jj,])
  ####
  return(mvc.data)
}

calib_init = function(t.init,mvc.data,sample=F,start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50){
  p.t = mvc.data$XT.data$p.t
  
  ll = sapply(1:nrow(t.init), function(j) fit_model(t.init[j,],mvc.data,lite=T,sample=sample,start.eta=start,end.eta=end,
...
calib_init_es = function(t.init,mvc.data,sample=F,n.samples=100,start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50){
  p.t = mvc.data$XT.data$p.t
...
calib_init_is = function(t.init,mvc.data,sample=F,n.samples=100,start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50){
  p.t = mvc.data$XT.data$p.t
...
mv_calib = function(mvc.data,init,nrestarts=1,sample=T,optimize=T,
                    ...
                    start.eta=6,end.eta=50,laGP.delta=F,start.delta=6,end.delta=50){
  ...
  if(optimize){
    ...
    for(i in 1:nrestarts){
      outi = snomadr(fit_model, n = mvc.data$XT.data$p.t, bbin = rep(0, mvc.data$XT.data$p.t), bbout = 0, 
                     x0 = as.numeric(init[i,2:ncol(init)]),
                     lb = rep(.01, mvc.data$XT.data$p.t), 
                     ub = rep(.99, mvc.data$XT.data$p.t),
                     opts = opts,
                     print.output=F,
                     mvc.data = mvc.data,
                     lite=T,negll=T,
                     ...
                     X.pred.eta=NULL,X.pred.delta=NULL)
      if(is.null(out) || outi$objective < out$objective) out <- outi
    }
    return = fit_model(out$solution,mvc.data,lite=F,sample=sample,start.eta=start.eta,end.eta=end.eta,
                       laGP.delta=laGP.delta,start.delta=start.delta,end.delta=end.delta)
    return$snomadr = out
    ...
    return = fit_model(init,mvc.data,lite=F,sample=sample,start.eta=start.eta,end.eta=end.eta,
                       laGP.delta=laGP.delta,start.delta=start.delta,end.delta=end.delta)
...
mv_calib_es = function(mvc.data,init,nrestarts=1,sample=T,optimize=T,n.samples=100,
                    opts=list("MAX_BB_EVAL" = 1000, "INITIAL_MESH_SIZE" = .1,
                              "MIN_POLL_SIZE" = "r0.001", "DISPLAY_DEGREE" = 0),
                    start=6,end=50,laGP.delta=F,start.delta=6,end.delta=50){
  ...
  if(optimize){
    ...
    out = NULL
    for(i in 1:nrestarts){
      outi = snomadr(energy_score_snomadr, n = mvc.data$XT.data$p.t, bbin = rep(0, mvc.data$XT.data$p.t), bbout = 0, 
                     x0 = as.numeric(init[i,2:ncol(init)]),
                     lb = rep(.01, mvc.data$XT.data$p.t), 
                     ub = rep(.99, mvc.data$XT.data$p.t),
                     opts = opts,
    ...
    return$es = energy_score_snomadr(init,mvc.data,sample=sample,n.samples=n.samples,
                                  start=start,end=end,laGP.delta=laGP.delta,start.delta=start.delta,end.delta=end.delta)
    return$theta.hat = init
    # TO DO: will this work for multivariate theta?
    return$theta.hat.orig = init * mvc.data$XT.data$sim$T$range + mvc.data$XT.data$sim$T$min
  }
...
mv_calib_is = function(mvc.data,init,nrestarts=1,sample=T,optimize=T,n.samples=100,
  ...
  if(optimize){
    ...
    out = NULL
    for(i in 1:nrestarts){
      outi = snomadr(interval_score_snomadr, n = mvc.data$XT.data$p.t, bbin = rep(0, mvc.data$XT.data$p.t), bbout = 0, 
                     x0 = as.numeric(init[i,2:ncol(init)]),
                     lb = rep(.01, mvc.data$XT.data$p.t), 
                     ub = rep(.99, mvc.data$XT.data$p.t),
                     opts = opts,
    ...
    return = interval_score_snomadr(out$solution,mvc.data,sample,n.samples,start,end,laGP.delta,start.delta,end.delta,lite=F)
    return$snomadr = out
    ...
    return = interval_score_snomadr(init,mvc.data,sample=sample,n.samples=n.samples,
                                     start=start,end=end,laGP.delta=laGP.delta,start.delta=start.delta,end.delta=end.delta,lite=F)
    return$theta.hat = init
    # TO DO: will this work for multivariate theta?
    return$theta.hat.orig = init * mvc.data$XT.data$sim$T$range + mvc.data$XT.data$sim$T$min
  }
...
do_mcmc = function(mvc.data,t.init,n.samples=100,n.burn=0,
  ...
  llt = fit_model(t,mvc.data,F,T,start.eta,end.eta,laGP.delta,start.delta,end.delta,X.pred.eta,X.pred.delta)
  ...
  for(i in 2:n.samples){
    ...
    for(j in 1:p.t){
      ...
      if(t.prop[j]<0 | t.prop[j]>1){
        ...
        if(get.pred[i]){
          ll.prop = fit_model(t.prop,mvc.data,F,T,start.eta,end.eta,laGP.delta,start.delta,end.delta,X.pred.eta,X.pred.delta)
        } else{
          ll.prop = fit_model(t.prop,mvc.data,F,T,start.eta,end.eta,laGP.delta,start.delta,end.delta,NULL,NULL)
        }
      }
      if(get.pred[i]){
        llt = fit_model(t,mvc.data,F,T,start.eta,end.eta,laGP.delta,start.delta,end.delta,X.pred.eta,X.pred.delta)
      } else{
        llt = fit_model(t,mvc.data,F,T,start.eta,end.eta,laGP.delta,start.delta,end.delta,NULL,NULL)
      }
  ...
  returns = list(t.samp=t.store[(n.burn+1):n.samples,],
                 ...
                 v.samp=NULL,
                 v.GPs=NULL,
                 acpt.ratio=colSums(accept)/n.samples,
                 time=mcmc.time,
                 prop.step=prop.step,
                 n.samples = n.samples,
                 n.burn = n.burn)
...
add_samples = function(mvc.data,mcmc.out,n.samples=100,
  ...
  prev.samples = mcmc.out$n.samples-mcmc.out$n.burn
  mcmc.out$acpt.ratio = (prev.samples*mcmc.out$acpt.ratio + n.samples*add.samples$acpt.ratio)/(prev.samples+n.samples)
  mcmc.out$time = mcmc.out$time + add.samples$time
  mcmc.out$n.samples = mcmc.out$n.samples + n.samples
  return(mcmc.out)
}

tune_step_sizes = function(mvc.data,n.samples,n.levels,target.accept.rate=NULL,min.step=.01,max.step=1,
                           start.eta=6,end.eta=50,laGP.delta=F,start.delta=6,end.delta=50){
  p.t = mvc.data$XT.data$p.t
...
fit_model = function(theta,mvc.data,
                     lite=T,sample=T,
  ...
  # Predict from emulator at [X.obs,theta]
  eta = fit_eta(theta,mvc.data,sample,start.eta,end.eta,X.pred.eta)
  ...
  ll = compute_ll(theta,eta,delta,sample,mvc.data)
...
fit_eta = function(theta,mvc.data,
  ...
  # transform theta by estimated length-scales
  theta.sc = lapply(1:n.pc, function(jj) theta/sqrt(mvc.data$est.ls$T[[jj]]))
  ...
  if(p.t>1){
    ...
    if(p.x>0){
      XT.sc = lapply(1:n.pc, function(jj) cbind(mvc.data$SC.inputs$X.obs[[jj]],theta.sc.rep[[jj]]))
    } else{
      XT.sc = theta.sc.rep
    }
  } else{
...
compute_ll = function(theta,eta,delta,sample,mvc.data){
  ...
  if(sample){
    ...
    ldetS = mvc.data$Y.data$obs$n.y*log(ssq.hat)
    Sinv = mvc.data$precomp$I.n.y/ssq.hat
    ...
    # I.n.y = eye(n.y)
    #B.hat = mvc.data$BDtBDinv%*%mvc.data$tBD%*%y.resid
    B.hat = mvc.data$precomp$BDtBDinvtBD%*%y.resid
    SigB.hat = lapply(1:mvc.data$Y.data$n, function(i) as.matrix(diag(c(eta$w$var[,i],delta$v$var[,i]),nrow=length(c(eta$w$var[,i],delta$v$var[,i]))) + ssq.hat*mvc.data$precomp$BDtBDinv))
    l_beta_hat = sapply(1:mvc.data$Y.data$n, function(i) dmvnorm(x=as.numeric(B.hat[,i]),sigma = SigB.hat[[i]], log = T))
    ll = sapply(1:mvc.data$Y.data$n, function(i) -.5*((mvc.data$Y.data$obs$n.y-mvc.data$precomp$rankBD)*log(2*pi*ssq.hat) + mvc.data$precomp$ldetBDtBD +
                                        lambda*t(y.resid[,i])%*%mvc.data$precomp$LLHmat%*%y.resid[,i]) + l_beta_hat[i])
  }
  return(sum(ll) + sum(dbeta(theta,2,2,log=T)) + dinvgamma(ssq.hat,.001,.001,log=T))# + log(1/ssq.hat))
}
